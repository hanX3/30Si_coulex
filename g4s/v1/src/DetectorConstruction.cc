...
G4VPhysicalVolume* DetectorConstruction::DefineVolumes()
{
  ...
  imp_hpge_array = new IMPHPGeArray(world_log);
  imp_hpge_array->Construct();
  imp_hpge_array->Report(); 

  // s3 si array
  G4Tubs *si_solid = new G4Tubs("si_solid", 0., 2.*(S3SiOuterRadius+1.*mm), 2.*m, 0., twopi);
  G4LogicalVolume *si_log = new G4LogicalVolume(si_solid, mat_vaccum, "si_log", nullptr, nullptr, nullptr);
  // new G4PVPlacement(nullptr, G4ThreeVector(), si_log, "si_phys", world_log, false, 0, check_overlaps);

  s3_si_array = new S3SiArray(world_log);
  ...
  si_sd = new SiSD("S3SiSD", "SiHitCollection");
  sd_manager->AddNewDetector(si_sd);
  SetSensitiveDetector("s3_si_pixel_log", si_sd, true);

  hpge_sd = new HPGeSD("HPGeSD", "HPGeHitCollection");
  sd_manager->AddNewDetector(hpge_sd);
  SetSensitiveDetector("hpge_crystal_log", hpge_sd, true);

  clover_sd = new HPGeSD("CloverSD", "HPGeHitCollection");
  sd_manager->AddNewDetector(clover_sd);
  SetSensitiveDetector("clover_crystal_log", clover_sd, true);
