...
void analysis_step1(int run_num)
{
  TFile *file_in;
  TString str_file_in;
  str_file_in = TString::Format("../ana_rootfile/run%05d_ana.root", run_num);
  ...
  for(int i=0;i<32;i++){
    ...
    h2[i] = new TH2D(str_h2_name.Data(), str_h2_title.Data(), 2048, 0, 32768, 2048, 0, 32768);
    ...
    n = tr1_Si->Draw(str_draw.Data(), str_cut.Data(), "col");
    cout << "n " << n << endl;
    gr[i] = new TGraph(n, tr1_Si->GetV2(), tr1_Si->GetV1());
    ...
    tf1[i] = new TF1(str_tf1_name.Data(), "pol1", 2000, 32768);
...
//sector par0 = 6.51727 par1 = 0.974135
//modified ring to sector1--mod7ch00
void analysis_step2(int run_num)
{
  TFile *file_in;
  TString str_file_in;
  str_file_in = TString::Format("../ana_rootfile/run%05d_ana.root", run_num);
  ...
  for(int i=18;i<24;i++){
    ...
    h2[i] = new TH2D(str_h2_name.Data(), str_h2_title.Data(), 2048, 0, 32768, 2048, 0, 32768);
    str_draw = TString::Format("Si_ring_adc:(6.51727+0.974135*Si_sector_adc)>>%s", str_h2_name.Data());
    ...
    n = tr1_Si->Draw(str_draw.Data(), str_cut.Data(), "col");
    cout << "n " << n << endl;
    gr[i] = new TGraph(n, tr1_Si->GetV1(), tr1_Si->GetV2());//attention
